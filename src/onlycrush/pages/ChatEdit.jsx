...
export default function ChatEdit() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const genRes = await api.post('/generate/chat-edit', { imageUrl, instruction });
      const taskId = genRes.data?.taskId || genRes.taskId;
      toast.success('Chat edit started!');
    ...
    <div className="min-h-screen bg-black pb-24">
      ...
      <div className="px-4 py-6 space-y-6">
        ...
        <div className="bg-[#141414] rounded-2xl p-4 border border-[#262626]">
          <label className="block text-white font-medium mb-2">Edit Instruction</label>
          <textarea
            value={instruction}
            onChange={(e) => setInstruction(e.target.value)}
            placeholder="e.g. Change background to beach sunset"
            className="w-full h-24 bg-[#1a1a1a] rounded-xl p-4 text-white placeholder-white/40 border border-[#262626] resize-none focus:outline-none focus:ring-2 focus:ring-purple-500"
          />
        </div>
        ...
        <button
          ...
          className={`w-full py-4 rounded-xl text-white font-bold flex items-center justify-center gap-2 ${loading || !image || !instruction.trim() ? 'bg-[#262626] cursor-not-allowed' : 'bg-gradient-to-r from-purple-600 to-pink-600'}`}
        >
          {loading ? (
            <>
